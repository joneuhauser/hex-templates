...
p=Path(__file__).resolve().parent;out=p/'experiments/orbit-seeds';out.mkdir(parents=True,exist_ok=True)
def read(path):
...
records=[]
for n in [36,44]:
 mesh=read(p/f'input/published{n}-normalized.mesh');points=mesh['Vertices'];boundary=points[:18];accepted=0
 for swap in [0,1]:
  for sx in [1,-1]:
   for sy in [1,-1]:
    ...
    cmd=[str(p/'build/mirror_search'),'--input',str(p/'input/pyramid.mesh'),'--mirrors','0','--cap',str(n),'--interior',str(33 if n==36 else 43),'--threads','1','--replay',str(stem)+'.mesh','--output',str(stem)+'.jsonl']
...
(p/'experiments/orbit-replays.json').write_text(json.dumps(records,indent=2))
