        ...
        for count in sorted({c['side_quads'] for c in selected}):
            ...
            for c in sorted((c for c in selected if c['side_quads'] == count), key=lambda c:(-c['side_quality'],c['id'])):
                ...
                for planes in ['axial', 'diagonal']:
                    ...
                    label = f'{planes.capitalize()}: complete through {complete}'
                    if partial > complete:
...
<meta name="description" content="Certified 9-to-1 and 16-to-4 hexahedral refinement templates and symmetric side-wall quadrangulations.">
...
<p>Transitions from a 3×3 quad grid to one quad (9 → 1), and from a 4×4 grid to a 2×2 grid (16 → 4). The fine grid is on top and the coarse grid below; the same connectivity can be used in either direction.</p>
<p>Five certified meshes and all sixteen tested side-wall patterns. Every mesh has strictly positive Jacobian determinants throughout its cells.</p>
...
<p><a href="?cells=13#F9-1-H13">F9-1-H13</a> and <a href="?cells=28#F16-4-H28">F16-4-H28</a> are the smallest fillings found for the tested side walls.
...
('Search limits', 'The six 9 → 1 boundaries are completely enumerated through 28 hexes in both orientations. The ten 16 → 4 boundaries are completely enumerated through 28 hexes in both orientations and through 36 with diagonal mirrors; some axial cap-36 searches are incomplete. These are fixed-boundary results, not unrestricted minimum cell counts. <a href="assets/refinement/searches.json">Search records</a> identify completed and time-limited runs.'),
('Side-wall search', 'Up to eight quads, with zero or one additional vertex on each vertical edge and left–right reflection. Top/bottom edges have 3/1 or 4/2 segments. Reflection-orbit enumeration agrees with single-quad enumeration. Top–bottom flips are distinct because the cap subdivisions differ.'),
...
('Literature', 'Published refinement methods include <a href="https://doi.org/10.1016/j.proeng.2015.10.120">Owen and Shih’s two-refinement templates</a> and <a href="https://arxiv.org/abs/2512.14862">Tong and Zhang’s three-refinement templates</a>. The meshes here are independently found by this search; connectivity equivalence to those template sets has not been established. No novelty or literature-wide optimality claim is made.'),
])}
...
<p>All sixteen patterns from the 2D search, grouped by transition and quad count. Within each group, the best minimum corner mean ratio comes first. The score is <code>2 det(u,v) / (|u|² + |v|²)</code> for outgoing corner edges u and v, minimized over all corners; it penalizes skew and stretching. These are the actual square side walls of [−1,1]³. A valid wall alone does not establish a valid volume filling.</p>
{''.join(walls)}
...
def build():
    ...
    dump(OUT/'catalog.json', catalog); dump(OUT/'side-catalog.json', dict(max_quads=8, patterns=cases))
    ...
    (OUT/'README.md').write_text('''# Refinement templates
