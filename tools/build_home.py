"""Build the collection landing page and data-derived SVG previews."""
...
def build():
    ...
    refinement=json.loads((DOCS/'assets/refinement/height/sj-catalog.json').read_text())
    ...
    refinement_sides=json.loads((DOCS/'assets/refinement/side-catalog.json').read_text())
    ...
    cards=[
        ...
        ('refinement.html','refinement.svg','Refinement templates','9-to-1, 16-to-4, 25-to-9 and 25-to-1 quad-grid transitions, with certified hex meshes and enumerated side walls.',f'{len(refinement["templates"])} meshes',f'{len(refinement_sides["patterns"])} side-wall patterns' ,'A 32-cell hexahedral refinement transition')]
